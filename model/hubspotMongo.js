...
const {readOAuth, insertOAuth, updateOAuth} = require('./mongo');
const REDIRECT_URI = ``; //Here goes your redirect url

const refreshAccessToken = async (portalID) => {
    const data = await readOAuth(portalID);
...
const getAccessToken = async (portalID) => {
    ...
    const timeDifference = currentTime - parseInt(data.timeStamp);
    ...
    return { token: data.access_token, portalID };
};

const isAuthorized = async (portalID) => {
    const data = await readOAuth(portalID);
    if (data) {
        return data.refresh_token ? true : false;
    } else {
        return false;
    }
};
...
const exchangeForTokens = async (exchangeProof) => {
    try {
        ...
        try {
            ...
            if (data > 0) {
                console.log('Update');
                await updateOAuth(portalID, tokens.access_token, tokens.refresh_token);
            } else {
                console.log('Insert');
                await insertOAuth(portalID, tokens.access_token, tokens.refresh_token);
            }
